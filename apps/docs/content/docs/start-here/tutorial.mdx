...
```bash
the-brain init --project my-react-app --work-dir ~/code/my-react-app --label "My React Project"
```
...
The daemon runs in the background, polling your IDE logs every 30 seconds. Verify:
...
Now use your AI coding assistant (Cursor, Claude Code, or Windsurf) inside `~/code/my-react-app`. Every interaction gets harvested:
...
# 1. Reads new interactions from IDE logs
...
```bash
the-brain context --prompt "fix the auth bug in login component" --format markdown
```
...
| Work | Use Cursor/Claude Code | IDE interactions harvested |
| Inspect | `the-brain inspect --stats` | See what was learned |
| Inject | `the-brain context --prompt "..."` | Get relevant context |

...
> **Important — LoRA adapters are model-bound.** An adapter trained on `mlx-community/gemma-4-e4b-it-OptiQ-4bit` only works with that exact base model. It's not a standalone model. Think of it as a set of weights that *bias* a specific model toward your style — the adapter is useless without the model it was trained on. See [MLX Training](/docs/customization/mlx-training) for a concrete Ollama/Gemma example.

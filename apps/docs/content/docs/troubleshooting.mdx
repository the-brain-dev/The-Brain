...
This happens on Macs with 16GB unified memory or less, especially with the default Gemma 4 model. The model + LoRA parameters + gradients + tokenized data exceed available GPU memory.
...
## Daemon Running But Not Harvesting (Stale Config Paths)

**Symptom:** Daemon starts without errors, status shows 🟢 running, but memory count never increases.

**Common after renames** (e.g., `my-brain` → `the-brain`): `config.json` was copied to the new location, but its **values** still point to old paths:

```bash
# Check for stale paths in config
rg '/Users/.*\.my-brain/' ~/.the-brain/config.json
```

**Fix:** Update every path field in `config.json` to `~/.the-brain/...`:
- `database.path`
- `daemon.logDir`
- `mlx.loraOutputDir`
- `wiki.outputDir`
- All project `dbPath`/`wikiDir`/`loraDir` entries

Then restart: `the-brain daemon stop && the-brain daemon start`
...
Open a [GitHub Discussion](https://github.com/the-brain-dev/Brain/discussions) with:

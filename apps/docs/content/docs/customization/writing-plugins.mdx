...
For the full hook system (17 lifecycle events + plugin-defined custom hooks), see [Hook System](/docs/core-concepts/hook-system).
...
| `TRAINING_START` | `{ model, fragmentCount }` |
| `TRAINING_COMPLETE` | `{ duration, loss, adapterPath }` |
| `DAEMON_START` | (none) |
| `DAEMON_STOP` | (none) |
...
import { createHookSystem, HookEvent } from "@the-brain-dev/core";
...
```json
{
  "name": "@the-brain-dev/plugin-<name>",
  "version": "0.1.0",
  "type": "module",
  "main": "./src/index.ts",
  "dependencies": {
